...
class Site Extends DBObject{
	...
	public function action_save() {		$this->user->have_access();
		if ($this->get(intval($_REQUEST['id']))) {			if ($this->getVariable('owner') == $this->user->objectId) {  				if ($_REQUEST['cat']) {  					$this->setVariable('owner',$this->user->objectId);
					...
					return TRUE;
				}else{					$this->session->set_notice('Не указанна ни одна кaтегория',ERROR);
					return FALSE;
				}
	...
	public function action_delete() {		$this->user->have_access();		if ($this->get(intval($_REQUEST['id']))) {			if ($this->getVariable('owner') == $this->user->objectId) {				$this->delete_dir_tree('data/sites/'.$this->objectId);				if ($this->delete()) {		        	$this->session->set_notice('Сайт успешно удален',OK);
	 		       	return TRUE;
