...
abstract class DBObject
{
    ...
    public function get($objectId = null, $SQL = null)
    {
        ...
        if ($this->objectId) {
          if (!$SQL) {
            $SQL = "SELECT * FROM `".$this->__table."` WHERE `".$this->__table."`.`".$this->__keyColumn."` = '".$this->objectId."' LIMIT 1";
        	}
    ...
    public function insert($keyval='', $debug=false)
    {
      if ($keyval) {
        $keyClause = "`".$this->__keyColumn."` = '".$keyval."',";
    	} else {
        $keyClause = '';
    	}
        ...
        if (strlen($setClause)>0) {
    			$SQL = "INSERT INTO `".$this->__table."` SET ".$keyClause." ".$setClause;
    	 		if ($this->DBM->ExecuteQuery($SQL)) {
            $insertId = mysqli_insert_id($this->db); // fix this
	 			    return $insertId;
	 		    }
        }
        return false;
    ...
    public function update()
    {
      if (isset($this->objectId)) {
            ...
            if (strlen($setClause)>0) {
              $SQL = "UPDATE `".$this->__table."` SET ".$setClause." WHERE `".$this->__keyColumn."` = '".$this->objectId."' LIMIT 1";
    ...
    public function increaseVariable($variableName, $increaseValue)
    {
      $this->setVariable($variableName, $this->getVariable($variableName)+$increaseValue);
    }
    public function decreaseVariable($variableName, $decreaseValue)
    {
          ...
          if (sizeof($this->_changedColumns)>0) {
                foreach ($this->_changedColumns as $columnName) {
                    if ($columnName != $this->__keyColumn && strval(intval($columnName))!=$columnName) {
                        $setClause .= "`".$columnName."` = '".mysqli_escape_string(mysqli_connect($this->sys['mysql_host'], $this->sys['mysql_user'], $this->sys['mysql_pass'], $this->sys['mysql_db']), $this->getVariable($columnName))."', ";
                    }
                }
            ...
            if (sizeof($this->data)>0) {
                foreach ($this->data as $columnName=>$columnValue) {
                    if ($columnName != $this->__keyColumn && strval(intval($columnName))!=$columnName) {
                        $setClause .= "`".$columnName."` = '".mysqli_escape_string(mysqli_connect($this->sys['mysql_host'], $this->sys['mysql_user'], $this->sys['mysql_pass'], $this->sys['mysql_db']), $columnValue)."', ";
                    }
                }
                $setClause = substr($setClause,0,-2);
            }
    ...
    public function load_tpl_vars()
    {
      if ($this->tpl_prefix && $this->data[$this->__keyColumn]) {
        foreach ($this->data as $key => $val) {
          $this->tpl->set($this->tpl_prefix.'_'.$key,$val);
			}
			return TRUE;
		}
		return FALSE;
